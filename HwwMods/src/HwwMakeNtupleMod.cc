// $Id: HwwMakeNtupleMod.cc,v 1.138 2013/07/13 08:06:26 ceballos Exp $
...
void HwwMakeNtupleMod::Process()
{
  ...
  if(fIsData == kFALSE){
    GenNeutrinos = GetObjThisEvt<MCParticleOArr>(ModNames::gkMCNeutrinosName);
...
/*
	...
	Bool_t genCut2  = GenLeptons->GetEntries() == 2 && GenLeptons->At(0)->Pt() > 20 && GenLeptons->At(1)->Pt() > 20 &&
							   GenLeptons->At(0)->AbsEta() < 2.5 && GenLeptons->At(1)->AbsEta() < 2.5;
	Bool_t genCut32 = GenLeptons->GetEntries() == 3 && GenLeptons->At(0)->Pt() > 20 && GenLeptons->At(1)->Pt() > 20 &&
							   GenLeptons->At(0)->AbsEta() < 2.5 && GenLeptons->At(1)->AbsEta() < 2.5 && 
	        					  (GenLeptons->At(2)->Pt() <= 10 || GenLeptons->At(2)->AbsEta() >= 2.5);
	Bool_t genCut3  = GenLeptons->GetEntries() == 3 && GenLeptons->At(0)->Pt() > 20 && GenLeptons->At(1)->Pt() > 20 && GenLeptons->At(2)->Pt() > 10 &&
							   GenLeptons->At(0)->AbsEta() < 2.5 && GenLeptons->At(1)->AbsEta() < 2.5 && GenLeptons->At(2)->AbsEta() < 2.5;
        if(genCut2 == true || genCut32 == true || genCut3 == true) printf("ZZZ %d %d %d %d\n",genCut2,genCut32,genCut3,leptonGenIsSeen[0]+leptonGenIsSeen[1]+leptonGenIsSeen[2]); 
	if(genCut3 == true && leptonGenIsSeen[0]+leptonGenIsSeen[1]+leptonGenIsSeen[2] == 2) {
          for (UInt_t j=0; j<3; j++) {
	    if(leptonGenIsSeen[j] == 0) printf("NNN %d %f %f\n",GenLeptons->At(j)->AbsPdgId(),GenLeptons->At(j)->Pt(),GenLeptons->At(j)->AbsEta());
            if(leptonGenIsSeen[j] == 1) printf("SSS %d %f %f\n",GenLeptons->At(j)->AbsPdgId(),GenLeptons->At(j)->Pt(),GenLeptons->At(j)->AbsEta());
          }
        }
      }
*/
	...
	if(leptons->GetEntries() > 2){
	 ...
	 processId = fMCEventInfo->ProcessId();
      ...
      if(higgsPt <= 0) { // if Higgs is not found, use leptons and neutrinos
        double thePH[2] = {0., 0.};
	if(GenLeptons){
          for (UInt_t i=0; i<GenLeptons->GetEntries(); ++i) {
            thePH[0] += GenLeptons->At(i)->Px(); thePH[1] += GenLeptons->At(i)->Py();
          }
        }
	if(GenNeutrinos){
	  for (UInt_t i=0; i<GenNeutrinos->GetEntries(); ++i) {
            thePH[0] += GenNeutrinos->At(i)->Px(); thePH[1] += GenNeutrinos->At(i)->Py();
          }
	}
        higgsPt = sqrt(thePH[0]*thePH[0]+thePH[1]*thePH[1]);
      }
	...
	if(fIsData == kTRUE) {
	  ...
	  fSmurfTree.auxVar0_     = nMaxMediumOtherBTagJet[0]; //NPU_Observed;
	...
	if(sortedJetsAll.size() >= 1) {
	  fSmurfTree.jet1McId_ = sortedJetsAll[0]->MatchedMCFlavor();
          Bool_t isTau = kFALSE;
	  ...
	  if(isTau == kTRUE) fSmurfTree.jet1McId_ += 100;
        }
