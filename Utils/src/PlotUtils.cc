...
// Rebin for 1D hists
//--------------------------------------------------------------------------------------------------
TH1F* PlotUtils::rebin(TH1F* hist, vector<double> xlowedges, const string name) {
...
//   cout << "Nbins: " << xlowedges.size() << endl;
  ...
  for (UInt_t binx=0; binx < xlowedges.size()+2 ;++binx) {
	...
	string name1;
	if(name == "")
		name1 = string(hist->GetName()) + "_rebinned_in_eff_tools";
	else
		name1 = name;
	

  TH1F *rebinHist = new TH1F(TString(name1), hist->GetTitle(), xlowedges.size()-1, xbins);
...
//--------------------------------------------------------------------------------------------------
TH1F* PlotUtils::rebin(TH1F* hist, Int_t nbins, const string name) {
  TH1F *result = hist;
  if (nbins > 0) {
...
// Rebin for 2D hists
//--------------------------------------------------------------------------------------------------
TH2F* PlotUtils::rebin(TH2F* hist, vector<double> xlowedges, vector<double> ylowedges, const string name) {
  ...
  TH2F *rebinHist = new TH2F(TString(name1), hist->GetTitle(), xlowedges.size() - 1, xLow, ylowedges.size() - 1, yLow);
...
// Rebin for 3D hists
//--------------------------------------------------------------------------------------------------
TH3F* PlotUtils::rebin(TH3F* hist, vector<double> xlowedges, vector<double> ylowedges, vector<double> zlowedges) {
...
THStack* PlotUtils::addAllSamplesStacked(vector<string> datasetFiles, 
                                         ...
                                         vector<double> bins = vector<double>(0), 
                                         string xAxisTitle = "",
                                         string yAxisTitle = "") {
...
void PlotUtils::drawStackedPlot(THStack *stackedHist , string plotname, vector<string> legendNames,
  ...
  TCanvas *cv = MitStyle::MakeCanvas("cv", plotname.c_str(), 800, 600);
...
TCanvas* PlotUtils::DrawDataSignalBkgHistogram(TH1F* data, TH1F* sig, THStack *bkg , TLegend *legend, 
                                           string plotname, 
                                           Bool_t useLogY = false, double MaxY = -99, 
                                           double MinX = -99, double MaxX = -99,
                                           double legendX1 = -99, double legendY1 = -99, 
                                           double legendX2 = -99, double legendY2 = -99) {
...
void PlotUtils::makeXSliceDistributionComparisonPlot( vector<TH2F*> hists,
  ...
  for(UInt_t i=1; int(i) <= hists[0]->GetXaxis()->GetNbins(); ++i) {
    ...
    leg->SetBorderSize(1);
    leg->SetTextSize(0.03);
    TCanvas *cv = MitStyle::MakeCanvas("cv", plotLabel.c_str(), 800, 900);
...
TH1F* PlotUtils::createEfficiencyHist(vector<string> datasetFiles, vector<string> datasetNames, string dirName,
                                      ...
                                      vector<Double_t> bins, Double_t xlow = -99, Double_t xhigh = -99, 
                                      Double_t ylow = -99, Double_t yhigh = -99 
  ) {
...
TGraphAsymmErrors* PlotUtils::createEfficiencyGraph(vector<string> datasetFiles, 
                                                    ...
                                                    vector<Double_t> bins, Int_t errorType = 0, 
                                                    Double_t xlow = -99, Double_t xhigh = -99, 
                                                    Double_t ylow = -99, Double_t yhigh = -99 
  ) {
...
// Create ROC curves
//--------------------------------------------------------------------------------------------------
TGraphAsymmErrors* PlotUtils::MakeSigEffVsBkgEffGraph(TH1F* signalHist, TH1F* bkgHist, std::string name, Bool_t cutBelow ) {
  //Make Met Plots
  const UInt_t nPoints = signalHist->GetXaxis()->GetNbins();
  double *SigEff = new double[nPoints];
  double *BkgEff = new double[nPoints];
  double *SigEffErrLow = new double[nPoints];
  double *SigEffErrHigh = new double[nPoints];
  double *BkgEffErrLow = new double[nPoints];
  double *BkgEffErrHigh = new double[nPoints];
  double NSigTotal = 0;
  double NBkgTotal = 0;
...
//
//*************************************************************************************************
TGraphAsymmErrors* PlotUtils::MakeCurrentWPSigEffVsBkgEffGraph(Double_t signalEff, Double_t bkgEff, std::string name ) {
  //Make Met Plots
  double SigEff[1];
  double BkgEff[1];
...
//
//*************************************************************************************************
TGraphAsymmErrors* PlotUtils::MakeSigEffVsCutValueGraph(TH1F* signalHist, std::string name, 
                                                        Bool_t cutBelow  ) {

  //Make Met Plots
  const UInt_t nPoints = signalHist->GetXaxis()->GetNbins();
  double *cutValue = new double[nPoints];
  double *cutValueErr = new double[nPoints];
  double *SigEff = new double[nPoints];
  double *SigEffErrLow = new double[nPoints];
  double *SigEffErrHigh = new double[nPoints];
  double NSigTotal = 0;
...
//
//*************************************************************************************************
TGraphAsymmErrors* PlotUtils::MakeCurrentWPSigEffVsCutValueGraph(TH1F* signalHist, std::string name, 
                                                                 Double_t myCutValue, 
                                                                 Bool_t cutBelow  ) {
  //Make Met Plots
  const UInt_t nPoints = signalHist->GetXaxis()->GetNbins();
